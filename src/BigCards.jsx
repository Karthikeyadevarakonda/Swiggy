...
const BigCards = ({ card ,isOpen,setIsOpen }) => {
  ...
  // console.log("RAMP",card)
  const {restaurant} = card;
  // console.log("THE REST INFO",restaurant)
  return (
   <div>
      {itemsList.map((card, index) => ( "categories" in card ? 
        ( <div key={index} >
          ...
          {isOpen === category.title && category.itemCards.map((obj, cardIndex) => (<MenuCard obj={obj} restaurant={restaurant} cardIndex={cardIndex} key={cardIndex}/>))}
          </div>
          ))}
          </div>
       ):(
          ...
          {isOpen === card.title && card.itemCards.map((obj, cardIndex) => (<MenuCard obj={obj}  restaurant={restaurant} cardIndex={cardIndex} key={cardIndex}/>))}
          </div>
         )
      ))}
    </div>
